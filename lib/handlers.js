...
handlers.favicon = function(data, cb){
    if(data.method == 'get') {
        ...
        handlers.getStaticAsset('favicon.ico', function(err, data){
            if(!err && data) {
                cb(200, data, 'favicon');
            } else {
                cb(500);
...
handlers.public = function(data, cb){
    if(data.method == 'get') {
        ...
        if(trimmedAssetName.length > 0) {
            ...
            handlers.getStaticAsset(trimmedAssetName, function(err, data){
                if(!err && data) {
                    // what mime type is it? Check the filename. Defaul to text.
