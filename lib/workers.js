...
workers.rotateLogs = function(){
    ...
    _logs.list(false /* include compressed */, function(err, logs){
...
workers.logPurgeLoop = function(){
    ...
    // delete old compressed logs every two minutes
    setTimeout(function(){
        workers.purgeCompressedLogs
    }, 1000 * 60 * 2);
...
workers.purgeCompressedLogs = function(){
    _logs.list(true /* include compressed */, function(err, logFilenames){
        if(!err && logFilenames && logFilenames.length > 1) {
            logFilenames.forEach(function(fileName){
                if(fileName.indexOf('.gz.b64') > -1){
                    // it is a compressed log
                    // was it compressed more than 7 days ago? Whack it.

                    // compressed logs are in format xxx-<date>[.log|.gz.b64|...],
                    // where <date> is the compression date. To determine how old the log is,
                    // split the filename on '.', and take the first element. 
                    // Split that on '-' and take the last element.
                    // That is the compression date.
                    ...
                    if(elems.length > 1) {
                        ...
                        if(elems.length==2){
                            try {
                                ...
                                debug('Could not purge log ' + fileName + '.'); 
                            }
                        }
                    }
