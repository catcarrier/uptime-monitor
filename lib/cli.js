...
e.on('more check info', function (str) {
    // get the checkid from the strng
    var arr = str.split('--');
    var checkId = typeof (arr[1]) == 'string' && arr[1].length > 0 ? arr[1] : false;
    if (checkId) {
        ...
        console.log("syntax: more check info --{checkId}");
    }
});
...
cli.responders.help = function () {
    var commands = {
        ...
        'list logs': 'Show list of log files (compressed only)',
...
cli.responders.moreCheckInfo = function (str) {
 ...
 // listLogs, re-implemented using a child process instead of the _data utility.
 // Original version is below.
cli.responders.listLogs = function () {

    var isWin = process.platform.toLowerCase().indexOf("win") > -1;
    var ls = isWin ? childProcess.spawn(process.env.comspec, ['/c', 'dir', '.logs',]) : childProcess.spawn('ls', ['./.logs/']);

    ls.stdout.on('data', function (dataObj) {
        // explode to separate lines
        var dataStr = dataObj.toString();
        var logFileNames = dataStr.split('\n');
        cli.verticalSpace();
        logFileNames.forEach(function (logFileName) {
            // compressed logs have a dash in the name
...
cli.responders.moreLogInfo = function (str) {
    // get the log file name from the param
    var arr = str.split('--');
    var logFileName = typeof (arr[1]) == 'string' && arr[1].length > 0 ? arr[1] : false;
    if (logFileName) {
        cli.verticalSpace();
        // decompress the log file
        _logs.decompress(logFileName, function (err, strData) {
            if (!err && strData && strData.length > 0) {
                // log is many json lines sep with newline
                // split it by line, into array
                var arr = strData.split('\n');
                arr.forEach(function (jsonStr) {
                    var logObject = helpers.parseJsonToObject(jsonStr);
        ...
        console.log("syntax: more log info --{logFileName minus ext}");
    }
}
